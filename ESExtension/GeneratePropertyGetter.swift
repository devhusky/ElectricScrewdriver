...
extension SourceEditorCommand {
    ...
    func generateGetter(with className: String, ivarName: String, protocolName: String) -> String? {
        ...
        case "UIStackView":
            return UIStackViewGetter(ivarName)
        default:
    ...
    func UITableViewGetter(_ ivarName: String) -> String {
        ...
        - (UITableView *)\(ivarName) {
            if (!_\(ivarName)) {
                _\(ivarName) = [[UITableView alloc] initWithFrame:CGRectZero];
                _\(ivarName).dataSource = self;
                _\(ivarName).delegate = self;
                _\(ivarName).showsVerticalScrollIndicator = NO;
                _\(ivarName).showsHorizontalScrollIndicator = NO;
                _\(ivarName).separatorStyle = UITableViewCellSeparatorStyleNone;
    ...
    func UIButtonGetter(_ ivarName: String) -> String {
        ...
        - (UIButton *)\(ivarName) {
            if (!_\(ivarName)) {
                ...
                _\(ivarName).titleLabel.font = [FFStyle fontPingFangWithSize:14];
    ...
    func UILabelGetter(_ ivarName: String) -> String {
        ...
        - (UILabel *)\(ivarName) {
            if (!_\(ivarName)) {
                ...
                _\(ivarName).font = [FFStyle fontPingFangWithSize:14];
                _\(ivarName).textColor = HexColor(0x333333);
            }
            return _\(ivarName);
        }
        """
    ...
    func UIStackViewGetter(_ ivarName: String) -> String {
        return """
        - (UIStackView *)\(ivarName) {
            if (!_\(ivarName)) {
                _\(ivarName) = [[UIStackView alloc] init];
                _\(ivarName).axis = UILayoutConstraintAxisHorizontal;
                _\(ivarName).distribution = UIStackViewDistributionEqualSpacing;
                _\(ivarName).alignment = UIStackViewAlignmentLeading;
                _\(ivarName).spacing = 10;
            }
            return _\(ivarName);
        }
        """
